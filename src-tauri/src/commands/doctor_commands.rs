...
use crate::models::{Doctor, CreateDoctorInput, UpdateDoctorInput};
...
#[tauri::command]
pub fn update_doctor(state: State<DbState>, input: UpdateDoctorInput) -> Result<Doctor, AppError> {
    let mut conn = state.conn.lock().map_err(|e| AppError::Internal(e.to_string()))?;
    doctor_repo::update(&mut conn, input)
}

#[tauri::command]
pub fn delete_doctor(state: State<DbState>, id: String) -> Result<(), AppError> {
    let mut conn = state.conn.lock().map_err(|e| AppError::Internal(e.to_string()))?;
    doctor_repo::delete(&mut conn, &id)
}
