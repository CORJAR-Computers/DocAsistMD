#![allow(dead_code)]
use crate::errors::AppError;use crate::models::Doctor;
...
fn map_doctor_row(row: DoctorRow) -> Doctor {
    Doctor {
        id: row.0, first_name: row.1, last_name: row.2, specialty: row.3,
        license_number: row.4, phone: row.5, email: row.6, schedule_start: row.7,
        schedule_end: row.8, working_days: row.9, status: row.10,
        created_at: row.11, updated_at: row.12,
    }
}

pub fn get_all(conn: &mut DbConnection) -> Result<Vec<Doctor>, AppError> {
    let rows: Vec<DoctorRow> = conn.query(
